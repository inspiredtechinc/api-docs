...
if [[ -n "$ENV" ]]; then
  for service in */ ; do
    ...
    if [ -f "$openapi_file" ]; then
      npx @redocly/cli build-docs "$openapi_file" -o "$service_name/$ENV/redoc.html"
      # Swagger UI HTML (fix: use correct Swagger UI embed and relative path)
      swagger_ui_html="$service_name/$ENV/swagger.html"
...
EOF
      ...
      date '+%Y-%m-%d %H:%M:%S.%3N' > "$service_name/$ENV/.generated"
    fi
  done
fi
...
echo "  <title>API Docs</title>" >> index.html
echo "  <style>" >> index.html
echo "    body { font-family: sans-serif; margin: 2em; background: #f7f7f7; color: #222; }" >> index.html
echo "    .env-group { display: flex; gap: 2em; margin-top: 2em; }" >> index.html
echo "    .card { border: 1px solid #ccc; border-radius: 8px; padding: 1.5em; min-width: 180px; box-shadow: 0 2px 8px #eee; background: #fafbfc; }" >> index.html
echo "    .card h2 { margin-top: 0; }" >> index.html
echo "    .card ul { list-style: none; padding: 0; }" >> index.html
echo "    .card li { margin: 0.5em 0; }" >> index.html
echo "    a { text-decoration: none; color: #0074d9; }" >> index.html
echo "    a:hover { text-decoration: underline; }" >> index.html
echo "    @media (prefers-color-scheme: dark) { body { background: #181a1b; color: #eee; } .card { background: #23272a; border-color: #444; } a { color: #66baff; } }" >> index.html
echo "  </style>" >> index.html
echo "  <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">" >> index.html
echo "</head>" >> index.html
echo "<body>" >> index.html
echo "  <h1>API Documentation</h1>" >> index.html
...
for service in */ ; do
  ...
  for env in prod staging; do
    if [ -f "$service_name/$env/redoc.html" ]; then
      has_env=true
      break
    fi
  done
  if [ "$has_env" = true ]; then
    echo "  <h2 style='margin-top:2em;'><a href='./$service_name/index.html' style=\"color:inherit;text-decoration:none;\">$service_name</a></h2>" >> index.html
    echo "  <div class=\"env-group\">" >> index.html
    for env in prod staging; do
      if [ -f "$service_name/$env/redoc.html" ]; then
        env_label="Production"
        [ "$env" = "staging" ] && env_label="Staging"
        # Read timestamp for this env, fallback to N/A
        if [ -f "$service_name/$env/.generated" ]; then
          updated=$(cat "$service_name/$env/.generated")
        else
          updated="N/A"
        fi
        echo "    <div class=\"card\">" >> index.html
        echo "      <h2>$env_label</h2>" >> index.html
        echo "      <ul>" >> index.html
        echo "        <li><a href='./$service_name/$env/redoc.html'>Redoc</a></li>" >> index.html
        echo "        <li><a href='./$service_name/$env/swagger.html'>Swagger UI</a></li>" >> index.html
        echo "      </ul>" >> index.html
        echo "      <p style=\"font-size:0.9em;color:#888;\">Last updated: $updated</p>" >> index.html
        echo "    </div>" >> index.html
      fi
    done
    echo "  </div>" >> index.html
    # --- Generate service-level index.html ---
    service_index="$service_name/index.html"
